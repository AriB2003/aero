...
class MyNode(Node):
    def __init__(self):
        ...
        # Initialize plane connection
        # self.loop.run_until_complete(self.setup_plane())

    # commented out right now because I don't want to connect to the plane

    # async def setup_plane(self):
    #     print("Connecting to plane...")
    #     await self.plane.connect(system_address="udp://:14550")
    #     self.get_logger().info("Waiting for plane connection...")
    #     async for state in self.plane.core.connection_state():
    #         if state.is_connected:
    #             self.get_logger().info("plane connected!")
    #             break

    # async def get_telemetry(self):
    #     position = await self.plane.telemetry.position().__aiter__().__anext__()
    #     attitude = await self.plane.telemetry.attitude_euler().__aiter__().__anext__()
