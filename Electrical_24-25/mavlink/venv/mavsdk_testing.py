...
async def run():
    ...
    orbit_height = position.absolute_altitude_m+1
    ...
    async for telemetry in drone.telemetry.position():
        print(f"Latitude: {telemetry.latitude_deg}, Longitude: {telemetry.longitude_deg}, Altitude: {telemetry.absolute_altitude_m}")
        await drone.action.do_orbit(radius_m=1,
                                velocity_ms=2,
