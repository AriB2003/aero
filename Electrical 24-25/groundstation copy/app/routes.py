...
def gradient_descent_path(start, waypoint, boundary_polygon, learning_rate=0.0001, max_steps=1000, tolerance=1e-6):
...
def calculate_path():
   ...
   data = request.json
   start = np.array([data['start'][0], data['start'][1]])
   waypoint = np.array([data['waypoint'][0], data['waypoint'][1]])
   ...
   if len(path) > 0:
       ...
       # Save GeoJSON
       output_path = 'path.geojson'
       with open(output_path, 'w') as f:
           json.dump(geojson, f)
      
       return jsonify({"status": "success", "message": "Path calculated", "file": output_path})
   else:
       return jsonify({"status": "failure", "message": "No valid path found"})


@app.route('/get-path', methods=['GET'])
def get_path():
   """Serve the GeoJSON path file."""
   path_file = 'path.geojson'
   if os.path.exists(path_file):
       return send_file(path_file, mimetype='application/json')
   else:
       return jsonify({"status": "failure", "message": "Path file not found"})
